...
static gd_status gd_reduce_sum_autograd_backward(gd_bwd_ctx *bwd,
                                                 const gd_tape_node *node)
{
    ...
    if (attrs != NULL && attrs->axis != GD_REDUCE_AXIS_ALL) {
        GD_TRY(gd_reduce_sum_axis_backward(bwd->ctx,
                                           x,
                                           &grad_out,
                                           attrs->axis,
                                           attrs->keepdims != 0U,
        ...
        GD_TRY(gd_reduce_sum_backward(bwd->ctx, x, &grad_out, &dx));
    }
    return gd_autograd_accumulate(bwd, x->id, &dx);
}

const gd_autograd_rule gd_bwd_rule_reduce_sum = {
    .kind = GD_OP_REDUCE_SUM,
    .name = "reduce_sum",
    .backward = gd_reduce_sum_autograd_backward,
};
