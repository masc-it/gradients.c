...
static gd_status gd_mul_autograd_backward(gd_bwd_ctx *bwd,
                                          const gd_tape_node *node)
{
    ...
    GD_TRY(gd_mul_backward(bwd->ctx,
                           x,
                           y,
                           &grad_out,
                           need_x ? &dx : NULL,
...
const gd_autograd_rule gd_bwd_rule_mul = {
    .kind = GD_OP_MUL,
    .name = "mul",
    .backward = gd_mul_autograd_backward,
};
