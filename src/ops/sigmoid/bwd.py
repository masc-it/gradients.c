...
"""Live gradients.c sigmoid autograd/backward correctness check against PyTorch."""
...
int main(int argc, char **argv)
{
    ...
    if (argc < 7 || parse_dtype(argv[4], &dtype, &elem_size) != 0 || parse_u32(argv[5], &rank) != 0 ||
        ...
        fprintf(stderr, "usage: %s X.bin G.bin DX.bin f16|f32 RANK DIM...\n", argv[0]);
        return 2;
    }
    for (i = 0U; i < rank; ++i) {
    ...
    cfg.scratch_slot_bytes = align_up(bytes * 6U + 1024U * 1024U, 4096U);
...
def compile_runner(root: Path, tmp: Path) -> Path:
    source = tmp / "gd_sigmoid_bwd_runner.c"
    binary = tmp / "gd_sigmoid_bwd_runner"
...
def make_cases() -> list[Case]:
    ...
    profile = os.environ.get("GD_SIGMOID_BWD_PROFILE", "smoke")
    if profile == "smoke":
        return smoke
    if profile == "classic":
        return classic
    ...
    if not selected:
        raise SystemExit(f"unknown GD_SIGMOID_BWD_PROFILE={profile!r}")
    return selected
...
def case_tensors(case: Case) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if case.edge:
        x = torch.tensor([-20.0, -12.0, -8.0, -4.0, -1.0, 0.0, 1.0, 4.0, 8.0, 12.0, 20.0], dtype=dtype)
        g = torch.tensor([0.25, -0.375, 0.5, -0.625, 0.75, -0.875, 1.0, -1.125, 1.25, -1.375, 1.5], dtype=dtype)
        return x.reshape(case.shape).contiguous(), g.reshape(case.shape).contiguous()
    gen = torch.Generator(device="cpu").manual_seed(4000 + sum(case.shape) * 23 + len(case.shape))
    x = (torch.randn(*case.shape, generator=gen, dtype=torch.float32) * 4.0).to(dtype).contiguous()
    g = (torch.randn(*case.shape, generator=gen, dtype=torch.float32) * 0.5).to(dtype).contiguous()
    return x, g
...
def run_case(runner: Path, root: Path, tmp: Path, case: Case) -> bool:
    ...
    cmd = [str(runner), str(x_path), str(g_path), str(dx_path), case.dtype, str(len(case.shape)), *(str(d) for d in case.shape)]
    proc = subprocess.run(cmd, cwd=root, env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if proc.returncode == 77:
        print(f"sigmoid bwd skipped case={case.name}: {proc.stdout.strip()}")
        return True
    if proc.returncode != 0:
        print(proc.stdout, end=""); print(proc.stderr, end="")
        raise RuntimeError(f"runner failed case={case.name} rc={proc.returncode}")
    actual = read_actual(dx_path, case)
    tol = 1.6e-3 if case.dtype == "f16" else 8.0e-6
    max_abs = float((actual.float() - ref.float()).abs().max().item())
    ok = max_abs <= tol
    print(f"sigmoid bwd actual {'ok' if ok else 'FAIL'} case={case.name} dtype={case.dtype} max_abs={max_abs:.3e}")
    return ok
...
def main() -> None:
    root = repo_root()
    build_library(root)
    with tempfile.TemporaryDirectory(prefix="gd-sigmoid-bwd-") as tmp_str:
        tmp = Path(tmp_str)
        runner = compile_runner(root, tmp)
        failures = sum(0 if run_case(runner, root, tmp, case) else 1 for case in make_cases())
        if failures:
            raise SystemExit(f"sigmoid bwd check failed cases={failures}")
