#include "../../backends/cpu_ref/cpu_op.h"

static gd_status sum_bwd_run(_gd_cpu_exec *exec, const _gd_node *node)
{
    gd_status status = GD_OK;
    void *go_data = NULL;
    void *out_data = NULL;
    const gd_tensor_desc *out_desc = NULL;

    status = _gd_cpu_exec_input(exec, node, 0, &go_data, NULL);
    if (status != GD_OK) {
        return status;
    }
    status = _gd_cpu_exec_output(exec, node, 0, &out_data, &out_desc);
    if (status != GD_OK) {
        return status;
    }
    status = _gd_cpu_require_f32(out_desc);
    if (status != GD_OK) {
        return status;
    }
    return _gd_cpu_k_sum_bwd(out_desc, out_data, go_data, node->attrs.dim, false);
}

const _gd_cpu_op _gd_cpu_op_sum_bwd = {
    .kind = _GD_OP_SUM_BWD,
    .name = "sum_bwd",
    .support = _gd_cpu_support_default,
    .run = sum_bwd_run,
};
