...
"""Live gradients.c MSE backward correctness check against PyTorch."""
...
int main(int argc, char **argv)
{
    ...
    if (argc < 10 || parse_dtype_with_size(argv[6], &dtype, &elem_size) != 0 || parse_f32(argv[7], &grad_value) != 0 ||
        parse_u32(argv[8], &rank) != 0 || rank == 0U || rank > GD_MAX_DIMS || argc != (int)(9U + rank)) {
        fprintf(stderr, "usage: %s X.bin Y.bin DX.bin DY.bin GRAD.bin f16|f32 grad RANK DIM...\n", argv[0]);
        return 2;
    }
    for (i = 0U; i < rank; ++i) {
    ...
    CHECK(ctx, gd_mse_backward(ctx, &x, &y, &grad, &dx, &dy));
    ...
    if (write_file(argv[3], dx_data, bytes) != 0 || write_file(argv[4], dy_data, bytes) != 0 ||
        write_file(argv[5], &grad_value, sizeof(grad_value)) != 0) { goto fail; }
    rc = 0;
    goto done;
fail:
    rc = 1;
...
def make_cases() -> list[Case]:
    ...
    profile = os.environ.get("GD_MSE_BWD_PROFILE", "smoke")
    if profile == "smoke":
        return smoke
    if profile == "classic":
        return classic
    ...
    if not selected:
        raise SystemExit(f"unknown GD_MSE_BWD_PROFILE={profile!r}")
    return selected


def case_inputs(case: Case) -> tuple[torch.Tensor, torch.Tensor]:
    dtype = torch.float16 if case.dtype == "f16" else torch.float32
    gen = torch.Generator(device="cpu").manual_seed(5000 + len(case.shape) * 131 + sum(case.shape))
    x = (torch.randn(*case.shape, generator=gen, dtype=torch.float32) * 1.25).to(dtype).contiguous()
    y = (torch.randn(*case.shape, generator=gen, dtype=torch.float32) * 1.25).to(dtype).contiguous()
    return x, y
...
def run_case(runner: Path, root: Path, tmp: Path, case: Case) -> bool:
    ...
    scale = case.grad_out * (2.0 / x.numel())
    ref_dx = (x.float() - y.float()) * scale
    ref_dy = -ref_dx
    if case.dtype == "f16":
    ...
    cmd = [str(runner), str(x_path), str(y_path), str(dx_path), str(dy_path), str(grad_path),
    ...
    if proc.returncode == 77:
        print(f"mse bwd skipped case={case.name}: {proc.stdout.strip()}")
        return True
    ...
    print(f"mse bwd {'ok' if ok else 'FAIL'} case={case.name} dtype={case.dtype} dx={max_dx:.3e} dy={max_dy:.3e}")
    return ok
...
def main() -> None:
    root = _REPO_ROOT
    build_library(root)
    with tempfile.TemporaryDirectory(prefix="gd-mse-bwd-") as tmp_str:
        tmp = Path(tmp_str)
        runner = compile_runner(root, tmp, "gd_mse_bwd_runner", RUNNER_SOURCE)
        failures = sum(0 if run_case(runner, root, tmp, case) else 1 for case in make_cases())
        if failures:
            raise SystemExit(f"mse bwd check failed cases={failures}")
