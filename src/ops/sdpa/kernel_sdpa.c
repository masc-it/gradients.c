...
gd_status _gd_cpu_k_sdpa_bwd(const gd_tensor_desc *q_desc, const void *q,
                             const gd_tensor_desc *k_desc, const void *k,
                             const gd_tensor_desc *v_desc, const void *v,
                             const gd_tensor_desc *bias_desc, const void *bias,
                             const void *go,
                             ...
                             float scale, int causal, int window, int prefix_len)
{
    ...
    for (j = 0; j < B * Tk * Hkv * Dh; ++j) {
        gd_status status = _gd_cpu_store_float(k_desc, dk, j, 0.0F);
        if (status != GD_OK) { return status; }
        status = _gd_cpu_store_float(v_desc, dv, j, 0.0F);
        if (status != GD_OK) { return status; }
    }
    for (j = 0; j < B * Tq * Hq * Dh; ++j) {
        gd_status status = _gd_cpu_store_float(q_desc, dq, j, 0.0F);
        if (status != GD_OK) { return status; }
    }
    ...
    for (b = 0; b < B; ++b) {
        for (hq = 0; hq < Hq; ++hq) {
            ...
            for (i = 0; i < Tq; ++i) {
                int64_t qbase = ((b * Tq + i) * Hq + hq) * Dh;
                double dqr[GD_SDPA_MAX_HEAD_DIM];
                double m = -HUGE_VAL;
                double sum = 0.0;
                ...
                for (j = 0; j < Tk; ++j) {
                    if (sdpa_allowed(i, j, Tq, Tk, causal, window, prefix_len)) {
                        ...
                        status = sdpa_bias_at_typed(bias_desc, bias, b, hq, i, j, &bv);
                        if (status != GD_OK) { return status; }
                        status = sdpa_dot_typed(q_desc, go, qbase, v_desc, v, kbase, Dh,
                                                &gv_dot_v);
                        if (status != GD_OK) { return status; }
                        p = exp((double)scale * dot + bv - m) / sum;
                        dsum += p * gv_dot_v;
                    }
                }
                for (j = 0; j < Tk; ++j) {
                    if (sdpa_allowed(i, j, Tq, Tk, causal, window, prefix_len)) {
                        ...
                        status = sdpa_bias_at_typed(bias_desc, bias, b, hq, i, j, &bv);
                        if (status != GD_OK) { return status; }
                        status = sdpa_dot_typed(q_desc, go, qbase, v_desc, v, kbase, Dh, &dp);
                        if (status != GD_OK) { return status; }
                        p = exp((double)scale * dot + bv - m) / sum;
                        ds = p * (dp - dsum);
                        for (c = 0; c < Dh; ++c) {
                            ...
                            float gv = 0.0F;
                            float old = 0.0F;
                            status = _gd_cpu_load_float(q_desc, q, qbase + c, &qv);
                            if (status != GD_OK) { return status; }
                            status = _gd_cpu_load_float(k_desc, k, kbase + c, &kv);
                            if (status != GD_OK) { return status; }
                            status = _gd_cpu_load_float(q_desc, go, qbase + c, &gv);
                            if (status != GD_OK) { return status; }
                            dqr[c] += (double)scale * ds * (double)kv;
                            status = _gd_cpu_load_float(k_desc, dk, kbase + c, &old);
                            if (status != GD_OK) { return status; }
                            status = _gd_cpu_store_float(k_desc, dk, kbase + c,
                                                         old + (float)((double)scale * ds *
                                                                       (double)qv));
                            if (status != GD_OK) { return status; }
                            status = _gd_cpu_load_float(v_desc, dv, kbase + c, &old);
                            if (status != GD_OK) { return status; }
                            status = _gd_cpu_store_float(v_desc, dv, kbase + c,
                                                         old + (float)(p * (double)gv));
                            if (status != GD_OK) { return status; }
                        }
                    }
                }
                for (c = 0; c < Dh; ++c) {
                    gd_status status = _gd_cpu_store_float(q_desc, dq, qbase + c, (float)dqr[c]);
                    if (status != GD_OK) { return status; }
                }
            }
        }
