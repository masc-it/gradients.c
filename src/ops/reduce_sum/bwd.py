...
int main(int argc, char **argv)
{
    ...
    CHECK(ctx, gd_tensor_empty(ctx, GD_ARENA_PARAMS, dtype, grad_rank, grad_rank == 0U ? NULL : grad_shape, 256U, &grad_out));
...
def compile_runner(root: Path, tmp: Path) -> Path:
    source = tmp / "gd_reduce_sum_bwd_runner.c"
    binary = tmp / "gd_reduce_sum_bwd_runner"
