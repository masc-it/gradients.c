...
int main(int argc, char **argv)
{
    ...
    if (argc < 9 || parse_dtype(argv[5], &dtype, &elem_size) != 0 ||
    ...
    cfg.scratch_slot_bytes = align_up(bytes * 8U + elems * 2U + 1024U * 1024U, 4096U);
...
def compile_runner(root: Path, tmp: Path) -> Path:
    source = tmp / "gd_dropout_bwd_runner.c"
    binary = tmp / "gd_dropout_bwd_runner"
