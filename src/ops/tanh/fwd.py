...
"""Live gradients.c tanh forward correctness check against PyTorch."""
...
def make_cases() -> list[Case]:
    ...
    profile = os.environ.get("GD_TANH_FWD_PROFILE", "smoke")
    if profile == "smoke":
        return smoke
    if profile == "classic":
        return classic
    ...
    if not selected:
        raise SystemExit(f"unknown GD_TANH_FWD_PROFILE={profile!r}")
    return selected
...
def case_input(case: Case) -> torch.Tensor:
    ...
    gen = torch.Generator(device="cpu").manual_seed(5000 + sum(case.shape) * 17 + len(case.shape))
...
def run_case(runner: Path, root: Path, tmp: Path, case: Case) -> bool:
    ...
    if proc.returncode == 77:
        print(f"tanh fwd skipped case={case.name}: {proc.stdout.strip()}")
        return True
    ...
    tol = 1.2e-3 if case.dtype == "f16" else 8.0e-6
    max_abs = float((actual.float() - ref.float()).abs().max().item())
    ok = max_abs <= tol
    print(f"tanh fwd actual {'ok' if ok else 'FAIL'} case={case.name} dtype={case.dtype} max_abs={max_abs:.3e}")
    return ok
...
def main() -> None:
    root = _REPO_ROOT
    build_library(root)
    with tempfile.TemporaryDirectory(prefix="gd-tanh-fwd-") as tmp_str:
        tmp = Path(tmp_str)
        runner = compile_runner(root, tmp, "gd_tanh_fwd_runner", RUNNER_SOURCE)
        failures = sum(0 if run_case(runner, root, tmp, case) else 1 for case in make_cases())
        if failures:
            raise SystemExit(f"tanh fwd check failed cases={failures}")
