...
"""Live gradients.c MSE forward correctness check against PyTorch."""
...
def make_cases() -> list[Case]:
    ...
    profile = os.environ.get("GD_MSE_FWD_PROFILE", "smoke")
    if profile == "smoke":
        return smoke
    if profile == "classic":
        return classic
    ...
    if not selected:
        raise SystemExit(f"unknown GD_MSE_FWD_PROFILE={profile!r}")
    return selected


def case_inputs(case: Case) -> tuple[torch.Tensor, torch.Tensor]:
    dtype = torch.float16 if case.dtype == "f16" else torch.float32
    gen = torch.Generator(device="cpu").manual_seed(4000 + len(case.shape) * 97 + sum(case.shape))
    x = (torch.randn(*case.shape, generator=gen, dtype=torch.float32) * 1.5).to(dtype).contiguous()
    y = (torch.randn(*case.shape, generator=gen, dtype=torch.float32) * 1.5).to(dtype).contiguous()
    return x, y
...
def run_case(runner: Path, root: Path, tmp: Path, case: Case) -> bool:
    ...
    if proc.returncode == 77:
        print(f"mse fwd skipped case={case.name}: {proc.stdout.strip()}")
        return True
    ...
    tol = 2.5e-4 if case.dtype == "f16" else 4.0e-6
    max_abs = abs(actual - float(ref.item()))
    ok = max_abs <= tol
    print(f"mse fwd {'ok' if ok else 'FAIL'} case={case.name} dtype={case.dtype} abs={max_abs:.3e}")
    return ok
...
def main() -> None:
    root = _REPO_ROOT
    build_library(root)
    with tempfile.TemporaryDirectory(prefix="gd-mse-fwd-") as tmp_str:
        tmp = Path(tmp_str)
        runner = compile_runner(root, tmp, "gd_mse_fwd_runner", RUNNER_SOURCE)
        failures = sum(0 if run_case(runner, root, tmp, case) else 1 for case in make_cases())
        if failures:
            raise SystemExit(f"mse fwd check failed cases={failures}")
