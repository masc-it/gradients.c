...
static inline uint gd_huber_sg_count(const uint requested)
{
    if (requested == 0u) {
        return 1u;
    }
    return requested > uint(GD_METAL_HUBER_MAX_SIMDGROUPS) ?
           uint(GD_METAL_HUBER_MAX_SIMDGROUPS) : requested;
}

static inline float gd_huber_value(const float d, const float delta)
{
    const float ad = fabs(d);
    return ad <= delta ? 0.5f * d * d : delta * (ad - 0.5f * delta);
}

static inline float gd_huber_grad(const float d, const float delta)
{
    const float ad = fabs(d);
    return ad <= delta ? d : (d < 0.0f ? -delta : delta);
}

static inline float gd_huber_finish(float simd_acc,
...
kernel void gd_huber_forward_f16_kernel(device const uchar *xbuf [[buffer(0)]],
                                        device const uchar *ybuf [[buffer(1)]],
                                        device uchar *outbuf [[buffer(2)]],
                                        constant gd_metal_huber_args &args [[buffer(3)]],
                                        uint3 tgpos [[threadgroup_position_in_grid]],
                                        uint simd_lane [[thread_index_in_simdgroup]],
                                        uint simdgroup_id [[simdgroup_index_in_threadgroup]])
{
    threadgroup float partials[GD_METAL_HUBER_MAX_SIMDGROUPS];
    const ulong chunk_id = ulong(tgpos.x);
    const uint simdgroups = gd_huber_sg_count(args.simdgroups);
    ...
    for (ulong i = begin + ulong(simdgroup_id) * 32ul + ulong(simd_lane); i < end;
        ...
        acc += gd_huber_value(float(x[i]) - float(y[i]), args.delta);
    }
    acc = simd_sum(acc);
    acc = gd_huber_finish(acc, partials, simd_lane, simdgroup_id, simdgroups);
    if (simdgroup_id == 0u && simd_lane == 0u) {
        out[chunk_id] = acc * args.scale;
    }
}

kernel void gd_huber_forward_f32_kernel(device const uchar *xbuf [[buffer(0)]],
                                        device const uchar *ybuf [[buffer(1)]],
                                        device uchar *outbuf [[buffer(2)]],
                                        constant gd_metal_huber_args &args [[buffer(3)]],
                                        uint3 tgpos [[threadgroup_position_in_grid]],
                                        ...
                                        uint simdgroup_id [[simdgroup_index_in_threadgroup]])
{
    ...
    for (ulong i = begin + ulong(simdgroup_id) * 32ul + ulong(simd_lane); i < end;
         i += ulong(simdgroups) * 32ul) {
        acc += gd_huber_value(x[i] - y[i], args.delta);
    }
    acc = simd_sum(acc);
    acc = gd_huber_finish(acc, partials, simd_lane, simdgroup_id, simdgroups);
    if (simdgroup_id == 0u && simd_lane == 0u) {
...
kernel void gd_huber_backward_f16_kernel(device const uchar *xbuf [[buffer(0)]],
                                         device const uchar *ybuf [[buffer(1)]],
                                         device const uchar *gradbuf [[buffer(2)]],
                                         device uchar *dxbuf [[buffer(3)]],
                                         device uchar *dybuf [[buffer(4)]],
                                         constant gd_metal_huber_args &args [[buffer(5)]],
                                         uint gid [[thread_position_in_grid]])
{
    ...
    const float d = gd_huber_grad(float(x[gid]) - float(y[gid]), args.delta) *
                    (grad[0] * args.scale);
    if (args.write_x != 0u) {
...
kernel void gd_huber_backward_f32_kernel(device const uchar *xbuf [[buffer(0)]],
                                         device const uchar *ybuf [[buffer(1)]],
                                         device const uchar *gradbuf [[buffer(2)]],
                                         device uchar *dxbuf [[buffer(3)]],
                                         device uchar *dybuf [[buffer(4)]],
                                         ...
                                         uint gid [[thread_position_in_grid]])
{
    ...
    device const float *y = reinterpret_cast<device const float *>(ybuf + args.y_offset);
    device const float *grad = reinterpret_cast<device const float *>(gradbuf + args.grad_out_offset);
    const float d = gd_huber_grad(x[gid] - y[gid], args.delta) * (grad[0] * args.scale);
    if (args.write_x != 0u) {
