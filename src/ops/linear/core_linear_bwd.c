                             ...
                             gd_tensor *grad_bias)
{
    ...
    st = gd_linear_backward_validate(ctx,
                                     x,
                                     w,
                                     bias,
                                     grad_out,
    ...
    if (need_grad_x) {
        ...
        if (!gd_linear_flat_matrix_view_from_tensor(&dx, info.rows, info.k, &dxv)) {
            return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT,
                                        "linear backward invalid grad_x view");
        }
    ...
    if (need_grad_w) {
        ...
        if (!gd_op_matrix_view_from_tensor(&dw, &dwv)) {
            return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT,
                                        "linear backward invalid grad_w view");
        }
    ...
    if (need_grad_bias) {
        ...
        if (!gd_op_vector_view_from_tensor(&db, &dbv)) {
            return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT,
                                        "linear backward invalid grad_bias view");
        }
