...
int main(int argc, char **argv)
{
    ...
    oracle_memory_config(&cfg,
                         align_up(x_bytes + w_bytes + b_bytes + g_bytes + 16U * 1024U * 1024U, 4096U),
                         align_up(dx_bytes + dw_bytes + b_bytes + 16U * 1024U * 1024U, 4096U));
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gd-linear-bwd-") as tmp_str:
        tmp = Path(tmp_str)
        runner = compile_runner(root, tmp, "gd_linear_bwd_runner", RUNNER_SOURCE)
        failures = 0
