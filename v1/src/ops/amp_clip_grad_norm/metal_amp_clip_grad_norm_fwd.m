...
static gd_status amp_clip_grad_norm_plan(_gd_metal_plan_ctx *ctx)
{
    ...
    ctx->exe->node_pso2[ctx->node_id] = (__bridge void *)_gd_metal_pipeline_named(ctx->state, "gd_amp_clip_norm_reduce");
    ctx->exe->node_pso3[ctx->node_id] = (__bridge void *)_gd_metal_pipeline_named(ctx->state, "gd_amp_clip_norm_scale");
    if (ctx->exe->node_pso2[ctx->node_id] == NULL || ctx->exe->node_pso3[ctx->node_id] == NULL) {
        return _gd_error(GD_ERR_BACKEND, "metal amp_clip_grad_norm kernels missing");
    }
    for (in_i = 2; in_i < ctx->node->n_inputs; ++in_i) {
    ...
    if (total_groups <= 0) {
        return _gd_error(GD_ERR_SHAPE, "amp_clip_grad_norm has no elements");
    }
...
static gd_status amp_clip_grad_norm_encode(_gd_metal_encode_ctx *ctx)
{
    ...
    id<MTLBuffer> scale = _gd_metal_value_buffer(exe, node->inputs[0]);
    id<MTLBuffer> found = _gd_metal_value_buffer(exe, node->inputs[1]);
    ...
    if (partial_pso == nil || reduce_pso == nil || scale_pso == nil || scratch == nil) {
        return _gd_error(GD_ERR_BACKEND, "metal amp_clip_grad_norm pipeline/scratch missing");
    }
    for (i = 2; i < node->n_inputs; ++i) {
        ...
        if (numel64 > INT_MAX || groups64 > INT_MAX || groups64 <= 0) {
            return _gd_error(GD_ERR_SHAPE, "amp_clip_grad_norm input too large");
        }
        if (total_groups > INT_MAX - (int)groups64) {
            return _gd_error(GD_ERR_SHAPE, "amp_clip_grad_norm has too many groups");
        }
    ...
    for (i = 2; i < node->n_inputs; ++i) {
        ...
        [enc setBuffer:scale offset:0 atIndex:2];
        [enc setBuffer:found offset:0 atIndex:3];
    ...
    for (;;) {
        ...
        [enc setBuffer:found offset:0 atIndex:2];
...
const _gd_metal_op _gd_metal_op_amp_clip_grad_norm = {
    .kind = _GD_OP_AMP_CLIP_GRAD_NORM,
    .name = "amp_clip_grad_norm",
    .plan = amp_clip_grad_norm_plan,
    .encode = amp_clip_grad_norm_encode,
};
