...
static gd_status linear_backward(_gd_bwd_ctx *b, const _gd_node *node)
{
    gd_status status = GD_OK;
    ...
    status = matmul_pair_backward(b, go, node->inputs[0], node->inputs[1], false,
...
const _gd_bwd_rule _gd_bwd_rule_linear = {
    .op = _GD_OP_LINEAR,
    .fn = linear_backward,
