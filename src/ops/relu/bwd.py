...
"""Live gradients.c ReLU autograd/backward correctness check against PyTorch."""
...
int main(int argc, char **argv)
{
    ...
    if (argc != 7 || parse_u32(argv[4], &rows) != 0 || parse_u32(argv[5], &cols) != 0 ||
        parse_u32(argv[6], &dtype) != 0) {
        fprintf(stderr, "usage: %s X.bin G.bin DX.bin ROWS COLS DTYPE\n", argv[0]);
        return 2;
    }
    ...
    x_data = malloc(bytes);
    g_data = malloc(bytes);
    dx_data = calloc(elems, elem_size);
    ...
    CHECK(ctx, gd_tensor_empty(ctx, GD_ARENA_PARAMS, (gd_dtype)dtype, 2U, shape, 256U, &g));
...
def run_case(runner: Path, root: Path, tmp: Path, case: Case) -> bool:
    ...
    g = (torch.randn(case.rows, case.cols, generator=gen, dtype=torch.float32) * 0.5).to(torch_dtype)
    ...
    ref = x_pt.grad.to(torch.float32)
    ...
    g.detach().cpu().contiguous().numpy().astype(np_dtype, copy=False).tofile(g_path)
    env = os.environ.copy()
    env["GRADIENTS_METALLIB"] = str(root / "build" / "gradients.metallib")
    proc = subprocess.run([str(runner), str(x_path), str(g_path), str(dx_path),
                           str(case.rows), str(case.cols), str(gd_dtype)],
    ...
    actual_np = np.fromfile(dx_path, dtype=np_dtype).reshape(case.rows, case.cols)
    actual = torch.from_numpy(actual_np.copy()).to(torch.float32)
    diff = (actual - ref).abs()
    max_abs = float(diff.max().item())
    ok = bool(torch.all(diff <= (1e-6 if case.dtype == "f32" else 0.0)).item())
    print(f"relu bwd actual {'ok' if ok else 'FAIL'} case={case.name} dtype={case.dtype} max_abs={max_abs:.3e}")
    return ok
