...
static gd_status sdpa_varlen_backward(_gd_bwd_ctx *b, const _gd_node *node)
{
    ...
    if (status != GD_OK) { return status; }
    status = _gd_bwd_fwd(b, node->inputs[3], &cu);
    if (status != GD_OK) { return status; }
    ...
    status = _gd_bwd_emit_multi(b, _GD_OP_SDPA_VARLEN_BWD, inputs, 5, &node->attrs,
                                out_descs, 3, grads);
    if (status != GD_OK) {
        return status;
    }
...
const _gd_bwd_rule _gd_bwd_rule_sdpa_varlen = {
    .op = _GD_OP_SDPA_VARLEN,
    .fn = sdpa_varlen_backward,
