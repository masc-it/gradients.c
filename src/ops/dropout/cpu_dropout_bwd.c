...
static gd_status dropout_bwd_run(_gd_cpu_exec *exec, const _gd_node *node)
{
    ...
    if (out_desc->dtype != GD_DTYPE_F32 && out_desc->dtype != GD_DTYPE_F16) {
        return _gd_error(GD_ERR_DTYPE, "dropout_bwd output must be F32 or F16");
    }
    return _gd_cpu_k_dropout_bwd(out_desc, out_data, go_data, node->attrs.scale,
                                 node->attrs.dropout_seed, _gd_cpu_exec_run_id(exec));
}

const _gd_cpu_op _gd_cpu_op_dropout_bwd = {
