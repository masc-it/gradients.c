...
static bool gd_swiglu_same_shape(const gd_tensor *x1, const gd_tensor *x2)
{
    uint32_t axis;
...
static gd_status gd_swiglu_validate_inputs(gd_context *ctx,
                                           const gd_tensor *x1,
                                           const gd_tensor *x2,
                                           size_t *out_count)
{
    ...
    *out_count = 0U;
    ...
    if (x1->dtype != GD_DTYPE_F16 || x2->dtype != GD_DTYPE_F16) {
        return gd_context_set_error(ctx,
                                    GD_ERR_UNSUPPORTED,
                                    "swiglu currently supports f16 tensors only");
    }
    if (!gd_swiglu_same_shape(x1, x2)) {
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "swiglu inputs must have equal shape");
    }
    if (!gd_tensor_is_contiguous(x1) || !gd_tensor_is_contiguous(x2)) {
        return gd_context_set_error(ctx,
                                    GD_ERR_UNSUPPORTED,
                                    "swiglu requires contiguous inputs");
    }
    st = gd_tensor_numel(x1, &numel);
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "swiglu invalid input shape");
    }
    if (numel <= 0 || (uint64_t)numel > (uint64_t)UINT32_MAX ||
        ...
        return gd_context_set_error(ctx,
                                    ...
                                    "swiglu element count unsupported");
    }
    *out_count = (size_t)numel;
    return GD_OK;
}

static gd_status gd_swiglu_validate_grad_out(gd_context *ctx,
                                             ...
                                             const gd_tensor *grad_out)
{
    ...
    if (grad_out->dtype != shape_like->dtype || !gd_swiglu_same_shape(shape_like, grad_out)) {
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "swiglu backward gradient shape/dtype mismatch");
    }
    if (!gd_tensor_is_contiguous(grad_out)) {
        return gd_context_set_error(ctx,
                                    GD_ERR_UNSUPPORTED,
                                    "swiglu backward requires contiguous grad_out");
    }
    return GD_OK;
}

static gd_status gd_swiglu_dispatch_forward(gd_context *ctx,
                                            const gd_tensor *x1,
                                            const gd_tensor *x2,
                                            gd_tensor *out)
{
    ...
    if (!gd_op_tensor_view_from_tensor(x1, &x1v) ||
        ...
        return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT, "swiglu invalid tensor view");
    }
    st = gd_backend_swiglu_forward(gd_context_backend(ctx), &x1v, &x2v, &ov);
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "backend swiglu forward failed");
    }
    return GD_OK;
}

static gd_status gd_swiglu_dispatch_backward(gd_context *ctx,
                                             const gd_tensor *x1,
                                             const gd_tensor *x2,
                                             const gd_tensor *grad_out,
                                             gd_tensor *grad_x1,
                                             gd_tensor *grad_x2)
{
    ...
    if (!gd_op_tensor_view_from_tensor(x1, &x1v) ||
        ...
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "swiglu backward invalid tensor view");
    }
    st = gd_backend_swiglu_backward(gd_context_backend(ctx),
                                    &x1v,
                                    &x2v,
                                    &gv,
    ...
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "backend swiglu backward failed");
    }
    return GD_OK;
}

static gd_status gd_swiglu_validate_split_input(gd_context *ctx,
                                                const gd_tensor *x12,
                                                ...
                                                size_t *out_count)
{
    ...
    *out_count = 0U;
    ...
    if (x12->dtype != GD_DTYPE_F16) {
        return gd_context_set_error(ctx,
                                    GD_ERR_UNSUPPORTED,
                                    "swiglu_split currently supports f16 tensors only");
    }
    if (x12->rank == 0U || x12->shape[x12->rank - 1U] <= 0 ||
        (x12->shape[x12->rank - 1U] & 1) != 0) {
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "swiglu_split requires an even nonzero last dimension");
    }
    if (!gd_tensor_is_contiguous(x12)) {
        return gd_context_set_error(ctx,
                                    GD_ERR_UNSUPPORTED,
                                    "swiglu_split requires contiguous input");
    }
    st = gd_tensor_numel(x12, &numel);
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "swiglu_split invalid input shape");
    }
    if (numel <= 0 || (numel & 1) != 0 || (uint64_t)(numel / 2) > (uint64_t)UINT32_MAX ||
        (uint64_t)(numel / 2) > (uint64_t)SIZE_MAX) {
        return gd_context_set_error(ctx,
                                    numel <= 0 ? GD_ERR_INVALID_ARGUMENT : GD_ERR_OUT_OF_MEMORY,
                                    "swiglu_split element count unsupported");
    }
...
static gd_status gd_swiglu_validate_split_grad(gd_context *ctx,
                                               const gd_tensor *x12,
                                               const gd_tensor *grad_out,
                                               size_t *out_count)
{
    ...
    st = gd_swiglu_validate_split_input(ctx, x12, out_shape, out_count);
    if (st != GD_OK) {
        return st;
    }
    st = gd_tensor_validate(ctx, grad_out);
    ...
    if (grad_out->dtype != x12->dtype || grad_out->rank != x12->rank) {
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "swiglu_split backward gradient shape/dtype mismatch");
    }
    for (axis = 0U; axis < x12->rank; ++axis) {
    ...
    if (!gd_tensor_is_contiguous(grad_out)) {
        return gd_context_set_error(ctx,
                                    GD_ERR_UNSUPPORTED,
                                    "swiglu_split backward requires contiguous grad_out");
    }
    return GD_OK;
}

static gd_status gd_swiglu_split_dispatch_forward(gd_context *ctx,
                                                  const gd_tensor *x12,
                                                  gd_tensor *out)
{
    ...
    if (!gd_op_tensor_view_from_tensor(x12, &x12v) ||
        ...
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "swiglu_split invalid tensor view");
    }
    st = gd_backend_swiglu_split_forward(gd_context_backend(ctx), &x12v, &ov);
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "backend swiglu_split forward failed");
    }
    return GD_OK;
}

static gd_status gd_swiglu_split_dispatch_backward(gd_context *ctx,
                                                   const gd_tensor *x12,
                                                   const gd_tensor *grad_out,
                                                   gd_tensor *grad_x12)
{
    ...
    if (!gd_op_tensor_view_from_tensor(x12, &x12v) ||
        ...
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "swiglu_split backward invalid tensor view");
    }
    st = gd_backend_swiglu_split_backward(gd_context_backend(ctx), &x12v, &gv, &dx12v);
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "backend swiglu_split backward failed");
    }
    return GD_OK;
}
                    ...
                    gd_tensor *out)
{
    ...
    st = gd_swiglu_validate_inputs(ctx, x1, x2, &count);
    if (st != GD_OK) {
        return st;
    }
    (void)count;
    ...
    st = gd_swiglu_dispatch_forward(ctx, x1, x2, &result);
    if (st != GD_OK) {
        return st;
    }
    {
                          ...
                          gd_tensor *out)
{
    ...
    st = gd_swiglu_validate_split_input(ctx, x12, out_shape, &count);
    if (st != GD_OK) {
        return st;
    }
    (void)count;
    ...
    result.is_leaf = false;
    st = gd_swiglu_split_dispatch_forward(ctx, x12, &result);
    if (st != GD_OK) {
        return st;
    }
    {
                                   ...
                                   gd_tensor *grad_x12)
{
    ...
    st = gd_swiglu_validate_split_grad(ctx, x12, grad_out, &count);
    if (st != GD_OK) {
        return st;
    }
    (void)count;
    ...
    dx12.is_leaf = false;
    st = gd_swiglu_split_dispatch_backward(ctx, x12, grad_out, &dx12);
    if (st != GD_OK) {
        return st;
    }
    *grad_x12 = dx12;
                             ...
                             gd_tensor *grad_x2)
{
    ...
    (void)count;
    st = gd_swiglu_validate_grad_out(ctx, x1, grad_out);
    if (st != GD_OK) {
        return st;
    }
    ...
    st = gd_swiglu_dispatch_backward(ctx,
                                     x1,
                                     x2,
                                     grad_out,
