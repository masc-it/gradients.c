...
static gd_status embedding_bwd_run(_gd_cpu_exec *exec, const _gd_node *node)
{
    ...
    return _gd_cpu_k_embedding_bwd(out_desc, out_data, go_desc, go_data, ids_desc, ids_data);
}

const _gd_cpu_op _gd_cpu_op_embedding_bwd = {
    .kind = _GD_OP_EMBEDDING_BWD,
    .name = "embedding_bwd",
    .support = _gd_cpu_support_default,
    .run = embedding_bwd_run,
};
