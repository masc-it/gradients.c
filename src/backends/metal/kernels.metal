...
/* ---- simdgroup_matrix (matrix-unit) GEMM -------------------------------- *
 * 64x64 output block per threadgroup; 4 simdgroups (2x2), each owning a 32x32
 * sub-block as a 4x4 array of 8x8 accumulator fragments. K streams in 8-deep
 * tiles staged into threadgroup memory (transpose/batch normalized during the
 * copy, so the simdgroup_load path always sees row-major As[BM][BK] / Bs[BK][BN]
 * and works for every shape the reference kernel accepted). Results are stored
 * to a threadgroup tile then bounds-copied to device. */
#define GD_SIMD_BM GD_METAL_GEMM_SIMD_BM
#define GD_SIMD_BN GD_METAL_GEMM_SIMD_BN
#define GD_SIMD_BK GD_METAL_GEMM_SIMD_BK
#define GD_SIMD_NTHREAD GD_METAL_GEMM_SIMD_THREADS

kernel void gd_matmul_simd(device const float *a              [[buffer(0)]],
                           device const float *b              [[buffer(1)]],
                           device float *out                  [[buffer(2)]],
                           constant gd_metal_matmul_params &p  [[buffer(3)]],
                           uint3 tgpos [[threadgroup_position_in_grid]],
...
{
    threadgroup float As[GD_SIMD_BM * GD_SIMD_BK];
    threadgroup float Bs[GD_SIMD_BK * GD_SIMD_BN];
    /* Tiny per-simdgroup 8x8 store staging keeps threadgroup memory small (high
     * occupancy); a full 64x64 output tile would dominate threadgroup memory and
     * throttle concurrent threadgroups. */
    threadgroup float Cstage[(GD_SIMD_NTHREAD / 32) * 64];

    int m0 = (int)tgpos.y * GD_SIMD_BM;
    int n0 = (int)tgpos.x * GD_SIMD_BN;
    int batch_lin = (int)tgpos.z;
    int sg_row = (int)sgid / 2;
    int sg_col = (int)sgid % 2;
    uint lane = tid % 32u;
    ...
    int n_tiles = (p.k + GD_SIMD_BK - 1) / GD_SIMD_BK;
    for (int t = 0; t < n_tiles; ++t) {
        int kbase = t * GD_SIMD_BK;
        for (int e = (int)tid; e < GD_SIMD_BM * GD_SIMD_BK; e += GD_SIMD_NTHREAD) {
            int mr = e / GD_SIMD_BK;
            int kr = e % GD_SIMD_BK;
            int gr = m0 + mr;
            int gk = kbase + kr;
            ...
            As[mr * GD_SIMD_BK + kr] = val;
        }
        for (int e = (int)tid; e < GD_SIMD_BK * GD_SIMD_BN; e += GD_SIMD_NTHREAD) {
            int kr = e / GD_SIMD_BN;
            int nc = e % GD_SIMD_BN;
            int gk = kbase + kr;
            int gc = n0 + nc;
            ...
            Bs[kr * GD_SIMD_BN + nc] = val;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        simdgroup_float8x8 afrag[4];
        simdgroup_float8x8 bfrag[4];
        for (int i = 0; i < 4; ++i) {
            simdgroup_load(afrag[i], As + (sg_row * 32 + i * 8) * GD_SIMD_BK, GD_SIMD_BK);
        }
        for (int j = 0; j < 4; ++j) {
            simdgroup_load(bfrag[j], Bs + (sg_col * 32 + j * 8), GD_SIMD_BN);
        }
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                simdgroup_multiply_accumulate(acc[i][j], afrag[i], bfrag[j], acc[i][j]);
            }
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }

    bool interior = (m0 + GD_SIMD_BM <= p.m) && (n0 + GD_SIMD_BN <= p.n);
    if (interior) {
        /* Fast path: store fragments straight to device memory (no threadgroup
         * staging or barriers). Covers all but the boundary blocks. */
        device float *o = out + batch_lin * p.out_mat + m0 * p.n + n0;
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                simdgroup_store(acc[i][j],
                                o + (sg_row * 32 + i * 8) * p.n + (sg_col * 32 + j * 8),
                                p.n);
            }
        }
    } else {
        threadgroup float *cst = Cstage + (int)sgid * 64;
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                simdgroup_store(acc[i][j], cst, 8);
                simdgroup_barrier(mem_flags::mem_threadgroup);
                for (int e = (int)lane; e < 64; e += 32) {
                    int rr = e / 8;
                    int cc = e % 8;
                    int gr = m0 + sg_row * 32 + i * 8 + rr;
                    int gc = n0 + sg_col * 32 + j * 8 + cc;
                    if (gr < p.m && gc < p.n) {
                        out[batch_lin * p.out_mat + gr * p.n + gc] = cst[rr * 8 + cc];
                    }
                }
                simdgroup_barrier(mem_flags::mem_threadgroup);
            }
...
kernel void gd_linear_simd(device const float *x               [[buffer(0)]],
                           device const float *w               [[buffer(1)]],
                           device const float *bias            [[buffer(2)]],
                           device float *out                   [[buffer(3)]],
                           constant gd_metal_linear_params &p   [[buffer(4)]],
                           ...
                           uint sgid   [[simdgroup_index_in_threadgroup]])
{
    threadgroup float Xs[GD_SIMD_BM * GD_SIMD_BK];
    threadgroup float Ws[GD_SIMD_BK * GD_SIMD_BN];
    threadgroup float Cstage[(GD_SIMD_NTHREAD / 32) * 64];

    int r0 = (int)tgpos.y * GD_SIMD_BM;
    int o0 = (int)tgpos.x * GD_SIMD_BN;
    int sg_row = (int)sgid / 2;
    int sg_col = (int)sgid % 2;
    uint lane = tid % 32u;
    ...
    int n_tiles = (p.in_features + GD_SIMD_BK - 1) / GD_SIMD_BK;
    for (int t = 0; t < n_tiles; ++t) {
        int kbase = t * GD_SIMD_BK;
        for (int e = (int)tid; e < GD_SIMD_BM * GD_SIMD_BK; e += GD_SIMD_NTHREAD) {
            int mr = e / GD_SIMD_BK;
            ...
            int gr = r0 + mr;
            int gk = kbase + kr;
            Xs[mr * GD_SIMD_BK + kr] = (gr < p.rows && gk < p.in_features)
                                           ? x[gr * p.in_features + gk] : 0.0f;
        }
        for (int e = (int)tid; e < GD_SIMD_BK * GD_SIMD_BN; e += GD_SIMD_NTHREAD) {
            int kr = e / GD_SIMD_BN;
            ...
            Ws[kr * GD_SIMD_BN + nc] = val;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        simdgroup_float8x8 xfrag[4];
        simdgroup_float8x8 wfrag[4];
        for (int i = 0; i < 4; ++i) {
            simdgroup_load(xfrag[i], Xs + (sg_row * 32 + i * 8) * GD_SIMD_BK, GD_SIMD_BK);
        }
        for (int j = 0; j < 4; ++j) {
            simdgroup_load(wfrag[j], Ws + (sg_col * 32 + j * 8), GD_SIMD_BN);
        }
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                simdgroup_multiply_accumulate(acc[i][j], xfrag[i], wfrag[j], acc[i][j]);
            }
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }

    bool interior = (r0 + GD_SIMD_BM <= p.rows) && (o0 + GD_SIMD_BN <= p.out_features);
    if (interior && !p.has_bias) {
        device float *o = out + r0 * p.out_features + o0;
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                simdgroup_store(acc[i][j],
                                o + (sg_row * 32 + i * 8) * p.out_features + (sg_col * 32 + j * 8),
                                p.out_features);
            }
        }
    } else {
        ...
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                ...
                for (int e = (int)lane; e < 64; e += 32) {
                    int rr = e / 8;
                    int cc = e % 8;
                    int gr = r0 + sg_row * 32 + i * 8 + rr;
                    int gc = o0 + sg_col * 32 + j * 8 + cc;
                    if (gr < p.rows && gc < p.out_features) {
                        out[gr * p.out_features + gc] =
                            (p.has_bias ? bias[gc] : 0.0f) + cst[rr * 8 + cc];
                    }
                }
                simdgroup_barrier(mem_flags::mem_threadgroup);
            }
        ...
        for (int qb = q_lo; qb < q_hi; qb += GD_SDPA_BK) {
            ...
            if (active) {
                for (int ii = 0; ii < tile; ++ii) {
                    ...
                    float dp = 0.0f;
                    for (int c = 0; c < p.Dh; ++c) {
                        ss += qsh[ii * p.Dh + c] * kreg[c];
                        dp += gsh[ii * p.Dh + c] * vreg[c];
                    }
                    ss = p.scale * ss + gd_sdpa_bias_at(bias, p, b, hq, i, j);
                    float pj = exp(ss - mm) / l;
                    float ds = pj * (dp - D);
                    for (int c = 0; c < p.Dh; ++c) {
                        dvacc[c] += pj * gsh[ii * p.Dh + c];
                        dkacc[c] += p.scale * ds * qsh[ii * p.Dh + c];
