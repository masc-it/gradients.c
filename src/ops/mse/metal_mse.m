...
static id<MTLBuffer> gd_mse_buffer(gd_backend_buffer *buffer)
{
    return (__bridge id<MTLBuffer>)buffer->buffer;
}

static size_t gd_mse_dtype_size(uint32_t dtype)
{
    if (dtype == (uint32_t)GD_DTYPE_F16) {
        return 2U;
    }
...
static bool gd_mse_byte_range_valid(const gd_backend_buffer *buffer,
                                    size_t offset,
                                    size_t nbytes)
{
...
static bool gd_mse_count_bytes(size_t count, uint32_t dtype, size_t *out_nbytes)
{
    size_t elem_size;
    if (out_nbytes == NULL || count == 0U) {
        return false;
    }
    elem_size = gd_mse_dtype_size(dtype);
...
static bool gd_mse_view_range_valid(const gd_backend_tensor_view *view)
{
    size_t nbytes;
    size_t elem_size;
    if (view == NULL || view->buffer == NULL || view->count == 0U ||
        !gd_mse_count_bytes(view->count, view->dtype, &nbytes)) {
        return false;
    }
    elem_size = gd_mse_dtype_size(view->dtype);
    return elem_size != 0U && view->offset % elem_size == 0U &&
           gd_mse_byte_range_valid(view->buffer, view->offset, nbytes);
}

static bool gd_mse_contiguous_view(const gd_backend_tensor_view *view)
{
    int64_t expected = 1;
    int32_t d;
...
static bool gd_mse_same_shape(const gd_backend_tensor_view *x,
...
static uint32_t gd_mse_simdgroups_for_chunk(uint64_t count)
{
    if (count <= 128U) {
        return 1U;
    }
    ...
    return GD_METAL_MSE_MAX_SIMDGROUPS;
}

static gd_status gd_mse_forward_validate(const gd_backend_tensor_view *x,
                                         const gd_backend_tensor_view *y,
                                         ...
                                         float scale)
{
    ...
    if (!(scale == scale) || chunk_size == 0U || chunk_size > UINT32_MAX ||
        !gd_mse_view_range_valid(x) || !gd_mse_view_range_valid(y) ||
        !gd_mse_view_range_valid(out) || x->dtype != y->dtype ||
        (x->dtype != (uint32_t)GD_DTYPE_F16 && x->dtype != (uint32_t)GD_DTYPE_F32) ||
        out->dtype != (uint32_t)GD_DTYPE_F32 || x->count > UINT32_MAX ||
        !gd_mse_same_shape(x, y) || !gd_mse_contiguous_view(x) ||
        !gd_mse_contiguous_view(y) || !gd_mse_contiguous_view(out)) {
                                 ...
                                 float scale)
{
    ...
    st = gd_mse_forward_validate(x, y, out, chunk_size, scale);
    if (st != GD_OK) {
        return st;
    }
    pso = gd_mse_forward_pso(backend, x->dtype);
    if (pso == nil) {
    ...
    args.scale = scale;
    args.simdgroups = gd_mse_simdgroups_for_chunk(logical_chunk);
    [encoder setComputePipelineState:pso];
    [encoder setBuffer:gd_mse_buffer(x->buffer) offset:0U atIndex:0U];
    [encoder setBuffer:gd_mse_buffer(y->buffer) offset:0U atIndex:1U];
    [encoder setBuffer:gd_mse_buffer(out->buffer) offset:0U atIndex:2U];
...
static bool gd_mse_grad_like_input(const gd_backend_tensor_view *x,
                                   const gd_backend_tensor_view *grad)
{
    return grad != NULL && gd_mse_view_range_valid(grad) && grad->dtype == x->dtype &&
           gd_mse_same_shape(x, grad) && gd_mse_contiguous_view(grad);
}

static gd_status gd_mse_backward_validate(const gd_backend_tensor_view *x,
                                          const gd_backend_tensor_view *y,
                                          ...
                                          float scale)
{
    if (!(scale == scale) || !gd_mse_view_range_valid(x) || !gd_mse_view_range_valid(y) ||
        !gd_mse_view_range_valid(grad_out) || x->dtype != y->dtype ||
        (x->dtype != (uint32_t)GD_DTYPE_F16 && x->dtype != (uint32_t)GD_DTYPE_F32) ||
        grad_out->dtype != (uint32_t)GD_DTYPE_F32 || grad_out->rank != 0U ||
        grad_out->count != 1U || x->count > UINT32_MAX || !gd_mse_same_shape(x, y) ||
        !gd_mse_contiguous_view(x) || !gd_mse_contiguous_view(y)) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    if (grad_x == NULL && grad_y == NULL) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    if (grad_x != NULL && !gd_mse_grad_like_input(x, grad_x)) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    if (grad_y != NULL && !gd_mse_grad_like_input(x, grad_y)) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    return GD_OK;
}
                                  ...
                                  float scale)
{
    ...
    st = gd_mse_backward_validate(x, y, grad_out, grad_x, grad_y, scale);
    if (st != GD_OK) {
        return st;
    }
    pso = gd_mse_backward_pso(backend, x->dtype);
    if (pso == nil) {
        return GD_ERR_UNSUPPORTED;
    }
    st = gd_metal_command_for_op(backend, &command_buffer, &immediate);
    ...
    [encoder setBuffer:gd_mse_buffer(x->buffer) offset:0U atIndex:0U];
    [encoder setBuffer:gd_mse_buffer(y->buffer) offset:0U atIndex:1U];
    [encoder setBuffer:gd_mse_buffer(grad_out->buffer) offset:0U atIndex:2U];
    [encoder setBuffer:gd_mse_buffer(grad_x != NULL ? grad_x->buffer : x->buffer) offset:0U atIndex:3U];
    [encoder setBuffer:gd_mse_buffer(grad_y != NULL ? grad_y->buffer : y->buffer) offset:0U atIndex:4U];
