...
static gd_status gd_sdpa_decode_validate_common(gd_context *ctx,
                                                const gd_tensor *q,
                                                ...
                                                gd_backend_sdpa_decode_args *args)
{
    gd_status st;
    uint32_t i;
    if (ctx == NULL || q == NULL || k_cache == NULL || v_cache == NULL || args == NULL) {
    ...
    if (!gd_tensor_is_contiguous(q) || !gd_tensor_is_contiguous(k_cache) ||
        !gd_tensor_is_contiguous(v_cache)) {
...
static gd_status gd_sdpa_decode_validate_at(gd_context *ctx,
                                            const gd_tensor *q,
                                            const gd_tensor *k_cache,
                                            const gd_tensor *v_cache,
                                            int32_t cache_pos,
                                            ...
                                            gd_backend_sdpa_decode_args *args)
{
    gd_status st = gd_sdpa_decode_validate_common(ctx, q, k_cache, v_cache, config, args);
    if (st != GD_OK) {
        return st;
    }
    if (cache_pos < 0 || (int64_t)cache_pos > k_cache->shape[1] ||
        q->shape[1] > k_cache->shape[1] - (int64_t)cache_pos) {
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "sdpa_decode_at cache_pos/Tq exceed cache length");
    }
                            ...
                            gd_tensor *out)
{
    ...
    if (ctx == NULL || q == NULL || k_cache == NULL || v_cache == NULL || out == NULL) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    st = gd_sdpa_decode_validate_at(ctx, q, k_cache, v_cache, cache_pos, config, &args);
    if (st != GD_OK) {
        return st;
    }
    shape[0] = q->shape[0];
    ...
    y.is_leaf = false;
    if (!gd_op_tensor_view_from_tensor(q, &qv) || !gd_op_tensor_view_from_tensor(k_cache, &kv) ||
        !gd_op_tensor_view_from_tensor(v_cache, &vv) || !gd_op_tensor_view_from_tensor(&y, &yv)) {
        return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT, "sdpa_decode_at invalid tensor view");
    }
