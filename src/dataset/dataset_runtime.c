...
static uint32_t gd_ds_get_le16(const uint8_t *p)
{
...
{
    return (uint64_t)gd_ds_get_le32(p) |
           ((uint64_t)gd_ds_get_le32(p + 4) << 32U);
}
...
gd_status gd_gdtok_read_header(const char *path, gd_gdtok_header *out)
{
    ...
    if (dtype != (uint32_t)GD_GDTOK_DTYPE_U16 &&
        dtype != (uint32_t)GD_GDTOK_DTYPE_U32) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    ...
    if (out->version != GD_GDTOK_VERSION || out->header_size != GD_GDTOK_HEADER_SIZE ||
        out->payload_offset != GD_GDTOK_HEADER_SIZE || out->block_len == 0U) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    return GD_OK;
}
...
static gd_status gd_gdtok_payload_nbytes(const gd_gdtok_header *h, size_t *out)
{
    size_t item_size;
    if (h == NULL || out == NULL) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    item_size = gd_gdtok_dtype_size(h->dtype);
    if (item_size == 0U) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    if (h->n_tokens > (uint64_t)(SIZE_MAX / item_size)) {
        return GD_ERR_OUT_OF_MEMORY;
    }
    *out = (size_t)h->n_tokens * item_size;
    return GD_OK;
}
                                     ...
                                     gd_gdtok_shard *out)
{
    ...
    status = gd_gdtok_payload_nbytes(&out->header, &payload_nbytes);
    if (status != GD_OK) {
        return status;
    }
...
static uint64_t gd_fnv64_bytes(uint64_t h, const void *data, size_t n)
{
...
static uint64_t gd_fnv64_u64(uint64_t h, uint64_t v)
{
    return gd_fnv64_bytes(h, &v, sizeof(v));
}

static uint64_t gd_fnv64_str(uint64_t h, const char *s)
{
    if (s == NULL) {
        return gd_fnv64_u64(h, 0U);
    }
    return gd_fnv64_bytes(h, s, strlen(s) + 1U);
}

static uint64_t gd_gdtok_num_samples(const void *impl_v)
{
    const gd_gdtok_dataset_impl *impl = (const gd_gdtok_dataset_impl *)impl_v;
    return impl != NULL ? impl->n_samples : 0U;
}

static uint64_t gd_gdtok_fingerprint(const void *impl_v)
{
    const gd_gdtok_dataset_impl *impl = (const gd_gdtok_dataset_impl *)impl_v;
    ...
    h = gd_fnv64_str(h, "gdtok-v1");
    h = gd_fnv64_u64(h, (uint64_t)impl->n_shards);
    h = gd_fnv64_u64(h, (uint64_t)impl->block_len);
    h = gd_fnv64_u64(h, (uint64_t)impl->vocab_size);
    h = gd_fnv64_u64(h, (uint64_t)impl->dtype);
    h = gd_fnv64_u64(h, impl->tokenizer_hash);
    h = gd_fnv64_u64(h, impl->n_samples);
    h = gd_fnv64_u64(h, impl->n_tokens);
    for (i = 0; i < impl->n_shards; ++i) {
        h = gd_fnv64_str(h, impl->shards[i].path);
        h = gd_fnv64_u64(h, impl->shards[i].header.n_tokens);
        h = gd_fnv64_u64(h, impl->shards[i].header.n_samples);
        h = gd_fnv64_u64(h, impl->shards[i].sample_base);
    }
    return h != 0U ? h : 1U;
}

static gd_status gd_gdtok_get_u64(const void *impl_v, const char *key, uint64_t *out)
{
    const gd_gdtok_dataset_impl *impl = (const gd_gdtok_dataset_impl *)impl_v;
...
gd_status gd_dataset_open_gdtok(const char **paths, int n_paths, gd_dataset **out)
{
    static const gd_dataset_ops ops = {
        "gdtok",
        gd_gdtok_num_samples,
        gd_gdtok_fingerprint,
        gd_gdtok_get_u64,
    ...
    *out = NULL;
    ...
    impl->shards = (gd_gdtok_shard *)calloc((size_t)n_paths, sizeof(*impl->shards));
    if (impl->shards == NULL) {
        free(impl);
        return GD_ERR_OUT_OF_MEMORY;
    }
    ...
    for (i = 0; i < n_paths; ++i) {
        ...
        if (shard->header.n_samples > (UINT64_MAX - 1U) / shard->header.block_len ||
            shard->header.n_tokens !=
                shard->header.n_samples * shard->header.block_len + 1U) {
            gd_gdtok_impl_destroy(impl);
            return GD_ERR_INVALID_ARGUMENT;
        }
...
static const gd_gdtok_shard *gd_gdtok_find_shard(const gd_gdtok_dataset_impl *impl,
...
static int32_t gd_gdtok_read_id(const gd_gdtok_shard *shard, uint64_t token_index)
{
    const uint8_t *p;
    ...
    if (shard->header.dtype == GD_GDTOK_DTYPE_U16) {
        return (int32_t)gd_ds_get_le16(p);
    }
    return (int32_t)gd_ds_get_le32(p);
}
                                          ...
                                          int32_t *targets)
{
    ...
    if (impl == NULL || impl->magic != GD_GDTOK_IMPL_MAGIC ||
        sample_index >= impl->n_samples) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    shard = gd_gdtok_find_shard(impl, sample_index, &local);
    if (shard == NULL) {
        return GD_ERR_INTERNAL;
    }
    start = local * (uint64_t)impl->block_len;
    for (j = 0U; j < impl->block_len; ++j) {
        int32_t tok = gd_gdtok_read_id(shard, start + (uint64_t)j);
        int32_t tgt = gd_gdtok_read_id(shard, start + (uint64_t)j + 1U);
        if (tok < 0 || tgt < 0) {
            return GD_ERR_INTERNAL;
        }
        tokens[j] = tok;
