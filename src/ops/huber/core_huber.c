...
static gd_status gd_huber_validate_inputs(gd_context *ctx,
                                          const gd_tensor *x,
                                          const gd_tensor *y,
                                          size_t *count_out)
{
    ...
    *count_out = 0U;
    ...
    if (x->dtype != GD_DTYPE_F16 && x->dtype != GD_DTYPE_F32) {
        return gd_context_set_error(ctx, GD_ERR_UNSUPPORTED,
                                    "huber supports f16/f32 tensors only");
    }
    if (y->dtype != x->dtype) {
        return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT,
                                    "huber input dtypes must match");
    }
    if (x->rank != y->rank) {
        return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT,
                                    "huber input shapes must match");
    }
    for (i = 0U; i < x->rank; ++i) {
    ...
    if (!gd_tensor_is_contiguous(x) || !gd_tensor_is_contiguous(y)) {
        return gd_context_set_error(ctx, GD_ERR_UNSUPPORTED,
                                    "huber requires contiguous inputs");
    }
    st = gd_tensor_numel(x, &numel);
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "huber invalid input shape");
    }
    if (numel <= 0 || (uint64_t)numel > (uint64_t)UINT32_MAX ||
        ...
        return gd_context_set_error(ctx,
                                    ...
                                    "huber element count unsupported");
    }
    *count_out = (size_t)numel;
    return GD_OK;
}

static gd_status gd_huber_validate_grad_out(gd_context *ctx, const gd_tensor *grad_out)
{
    gd_status st;
    ...
    if (grad_out->dtype != GD_DTYPE_F32 || grad_out->rank != 0U ||
        ...
        return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT,
                                    "huber backward requires scalar f32 grad_out");
    }
    return GD_OK;
}

static gd_status gd_huber_dispatch_forward(gd_context *ctx,
                                           const gd_tensor *x,
                                           const gd_tensor *y,
                                           const gd_tensor *out,
                                           size_t chunk_size,
                                           ...
                                           float delta)
{
    ...
    gd_status st;
    if (ctx == NULL || x == NULL || y == NULL || out == NULL || chunk_size == 0U ||
        chunk_size > (size_t)UINT32_MAX || !(scale == scale) || !(delta > 0.0f)) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    if (!gd_op_tensor_view_from_tensor(x, &xv) || !gd_op_tensor_view_from_tensor(y, &yv) ||
        !gd_op_tensor_view_from_tensor(out, &ov)) {
        return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT,
                                    "huber forward invalid tensor view");
    }
    st = gd_backend_huber_forward(gd_context_backend(ctx),
                                  &xv,
                                  &yv,
                                  &ov,
                                  (uint64_t)chunk_size,
                                  ...
                                  delta);
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "backend huber forward failed");
    }
    return GD_OK;
}

static gd_status gd_huber_dispatch_reduce(gd_context *ctx,
                                          ...
                                          float scale)
{
    ...
    if (!gd_op_tensor_view_from_tensor(partial, &pv) || !gd_op_tensor_view_from_tensor(out, &ov)) {
        return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT,
                                    "huber reduce invalid tensor view");
    }
    st = gd_backend_reduce_contiguous(gd_context_backend(ctx), &pv, &ov, scale);
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "backend huber reduce failed");
    }
    return GD_OK;
}

static gd_status gd_huber_dispatch_backward(gd_context *ctx,
                                            const gd_tensor *x,
                                            const gd_tensor *y,
                                            ...
                                            float delta)
{
    ...
    if (ctx == NULL || x == NULL || y == NULL || grad_out == NULL ||
        (grad_x == NULL && grad_y == NULL) || !(scale == scale) || !(delta > 0.0f)) {
        return GD_ERR_INVALID_ARGUMENT;
    }
    ...
    if (!gd_op_tensor_view_from_tensor(x, &xv) || !gd_op_tensor_view_from_tensor(y, &yv) ||
        ...
        return gd_context_set_error(ctx, GD_ERR_INVALID_ARGUMENT,
                                    "huber backward invalid tensor view");
    }
    st = gd_backend_huber_backward(gd_context_backend(ctx),
                                   &xv,
                                   &yv,
                                   &gv,
                                   ...
                                   delta);
    if (st != GD_OK) {
        return gd_context_set_error(ctx, st, "backend huber backward failed");
    }
    return GD_OK;
}
                   ...
                   gd_tensor *out)
{
    ...
    st = gd_huber_validate_inputs(ctx, x, y, &count);
    if (st != GD_OK) {
        return st;
    }
    ...
    chunk_count = (count + (size_t)GD_METAL_HUBER_CHUNK_SIZE - 1U) /
                  (size_t)GD_METAL_HUBER_CHUNK_SIZE;
    if (chunk_count <= 1U) {
        st = gd_huber_dispatch_forward(ctx,
                                       x,
                                       y,
                                       &result,
                                       count,
                                       inv_count,
                                       GD_HUBER_DEFAULT_DELTA);
    } else {
        gd_tensor partial;
        int64_t partial_shape[1];
        if (chunk_count > (size_t)INT64_MAX) {
            return gd_context_set_error(ctx, GD_ERR_OUT_OF_MEMORY,
                                        "huber partial count overflow");
        }
        ...
        if (st == GD_OK) {
            st = gd_huber_dispatch_reduce(ctx, &partial, &result, inv_count);
        }
    }
    if (st != GD_OK) {
        return st;
                            ...
                            gd_tensor *grad_y)
{
    ...
    st = gd_huber_validate_grad_out(ctx, grad_out);
    if (st != GD_OK) {
        return st;
    }
    ...
    st = gd_huber_dispatch_backward(ctx,
                                    x,
                                    y,
                                    grad_out,
                                    grad_x != NULL ? &dx : NULL,
