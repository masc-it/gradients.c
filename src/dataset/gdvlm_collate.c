...
static gd_status gd_vlm_u64_to_i64_dim(uint64_t value, int64_t *out)
{
    if (out == NULL || value == 0U || value > (uint64_t)INT64_MAX) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM batch dimension");
    }
    *out = (int64_t)value;
    return GD_OK;
}
                                        ...
                                        int *seq_out)
{
    ...
    if (dataset == NULL || prefix_out == NULL || max_text_out == NULL || seq_out == NULL) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM length args");
    }
    ...
    if (prefix_u64 == 0U || prefix_u64 > (uint64_t)INT_MAX ||
        ...
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM dataset lengths");
    }
    prefix = (int)prefix_u64;
    max_text = cfg != NULL && cfg->max_text_len > 0 ? cfg->max_text_len : (int)max_text_u64;
    if (max_text <= 0) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM max_text_len");
    }
    if (cfg != NULL && cfg->sequence_len > 0) {
        seq = cfg->sequence_len;
        if (seq < prefix + max_text) {
            return _gd_error(GD_ERR_INVALID_ARGUMENT, "VLM sequence_len too small");
        }
    } else {
        if (prefix > INT_MAX - max_text) {
            return _gd_error(GD_ERR_OUT_OF_MEMORY, "VLM sequence length overflow");
        }
        seq = prefix + max_text;
    }
    *prefix_out = prefix;
                                   ...
                                   int *n_fields_out)
{
    ...
    if (dataset == NULL || fields == NULL || n_fields_out == NULL || batch_size <= 0) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM batch field args");
    }
    if (field_cap < GD_VLM_BATCH_FIELD_COUNT) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "insufficient VLM batch field capacity");
    }
                                      ...
                                      int *idx_out)
{
    ...
    if (batch == NULL || name == NULL || idx_out == NULL) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM field args");
    }
    idx = gd_batch_field_index(batch, name);
    if (idx < 0 || gd_batch_field_dtype(batch, idx) != dtype ||
        gd_batch_field_rank(batch, idx) != rank) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM batch field schema");
    }
    if (rank > 0 && gd_batch_field_dim(batch, idx, 0) != dim0) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM batch dim0");
    }
    if (rank > 1 && gd_batch_field_dim(batch, idx, 1) != dim1) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM batch dim1");
    }
    if (rank > 2 && gd_batch_field_dim(batch, idx, 2) != dim2) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM batch dim2");
    }
    *idx_out = idx;
    return GD_OK;
}
...
static gd_status gd_vlm_validate_collate_config(const gd_vlm_collate_config *cfg)
{
    ...
    if (cfg->target_mode != GD_VLM_TEXT_TARGET_NEXT &&
        ...
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM target mode");
    }
    return GD_OK;
}
                           ...
                           void *user_data)
{
    ...
    if (dataset == NULL || sample_ids == NULL || batch == NULL || batch_size <= 0) {
        return _gd_error(GD_ERR_INVALID_ARGUMENT, "invalid VLM collate args");
    }
    ...
    if (patch_nbytes_u64 > (uint64_t)SIZE_MAX) {
        return _gd_error(GD_ERR_OUT_OF_MEMORY, "VLM patch batch too large");
    }
    ...
    if (patches == NULL || tokens == NULL || targets == NULL || positions == NULL ||
        ...
        return _gd_error(GD_ERR_INTERNAL, "VLM batch host field is null");
    }
    for (b = 0; b < batch_size; ++b) {
        ...
        if (info.token_len > (uint32_t)max_text && truncate_text == 0) {
            return _gd_error(GD_ERR_INVALID_ARGUMENT, "VLM sample text exceeds max_text_len");
        }
