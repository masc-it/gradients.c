                                        ...
                                        const gd_backend_tensor_view *out)
{
    ...
    if (!gd_sdpa_decode_view_range_valid(q) || !gd_sdpa_decode_view_range_valid(k) ||
        !gd_sdpa_decode_view_range_valid(v) || !gd_sdpa_decode_view_range_valid(pos) ||
        !gd_sdpa_decode_view_range_valid(out) || !gd_sdpa_decode_contiguous_valid(q) ||
        !gd_sdpa_decode_contiguous_valid(k) || !gd_sdpa_decode_contiguous_valid(v) ||
        !gd_sdpa_decode_contiguous_valid(pos) || !gd_sdpa_decode_contiguous_valid(out)) {
        return false;
    }
                                           ...
                                           const gd_backend_sdpa_decode_args *args)
{
    ...
    p->pos_offset = (uint64_t)pos->offset;
