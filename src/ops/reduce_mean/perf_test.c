...
static bool reduce_perf_status_ok(gd_context *ctx, gd_status st, const char *expr)
{
    ...
    fprintf(stderr,
            "[REDUCE_MEAN][FAIL] %s -> %s (%d), context_error=%s\n",
            expr,
            gd_status_string(st),
            (int)st,
    ...
    if (keepdims) {
        *out_rank = pcase->rank;
        for (dim = 0U; dim < pcase->rank; ++dim) {
            out_shape[dim] = dim == (uint32_t)axis ? 1 : pcase->shape[dim];
        }
        return true;
    }
...
static bool reduce_perf_create_model(const reduce_perf_case *pcase, reduce_perf_model *model)
{
    ...
    scalar_dtype = pcase->dtype == GD_DTYPE_F16 ? GD_DTYPE_F32 : pcase->dtype;
    model->scalar_elem_size = reduce_perf_dtype_size(scalar_dtype);
    if (model->elem_size == 0U || model->scalar_elem_size == 0U ||
        !reduce_perf_count(pcase->rank, pcase->shape, &model->x_count) ||
        !reduce_perf_axis_shape(pcase, false, &model->axis_rank, model->axis_shape) ||
        !reduce_perf_count(model->axis_rank, model->axis_shape, &model->axis_count) ||
        !reduce_perf_axis_shape(pcase, true, &model->axis_keep_rank, model->axis_keep_shape) ||
        !reduce_perf_count(model->axis_keep_rank, model->axis_keep_shape, &model->axis_keep_count)) {
        fprintf(stderr, "[REDUCE_MEAN][FAIL] invalid case=%s\n", pcase->name);
        return false;
    }
    if (model->x_count > SIZE_MAX / model->elem_size || model->axis_count > SIZE_MAX / model->elem_size ||
        model->axis_keep_count > SIZE_MAX / model->elem_size) {
        return false;
    }
    model->x_bytes = model->x_count * model->elem_size;
    model->axis_bytes = model->axis_count * model->elem_size;
    model->axis_keep_bytes = model->axis_keep_count * model->elem_size;
    params_bytes = reduce_perf_align_up(model->x_bytes + model->axis_bytes + model->axis_keep_bytes +
                                             64U * 1024U * 1024U,
                                         4096U);
    ...
    if (st == GD_ERR_UNSUPPORTED) {
        printf("[REDUCE_MEAN] skipped case=%s: no supported GPU backend\n", pcase->name);
        return false;
    }
    ...
    REDUCE_PERF_REQUIRE_OK(model->ctx,
                           gd_tensor_empty(model->ctx, GD_ARENA_PARAMS, scalar_dtype, gd_shape_make(0U, NULL), 256U, &model->scalar_grad));
    if (!reduce_perf_write_scalar_one(model->ctx, &model->scalar_grad, scalar_dtype)) {
        return false;
    }
    REDUCE_PERF_REQUIRE_OK(model->ctx,
    ...
    REDUCE_PERF_REQUIRE_OK(model->ctx,
                           gd_tensor_rand_uniform(model->ctx, GD_ARENA_PARAMS, pcase->dtype, gd_shape_make(model->axis_keep_rank, model->axis_keep_rank == 0U ? NULL : model->axis_keep_shape), 256U, 3003U, -0.5f, 0.5f, &model->axis_keep_grad));
...
static bool reduce_perf_run_axis_forward_keepdims(reduce_perf_model *model, int32_t axis)
{
    gd_tensor out;
    model->x.requires_grad = false;
    REDUCE_PERF_REQUIRE_OK(model->ctx, gd_begin_step(model->ctx, GD_SCOPE_EVAL, gd_batch_empty()));
...
static bool reduce_perf_run_axis_backward_keepdims(reduce_perf_model *model, int32_t axis)
{
    gd_tensor dx;
    model->x.requires_grad = false;
    REDUCE_PERF_REQUIRE_OK(model->ctx, gd_begin_step(model->ctx, GD_SCOPE_EVAL, gd_batch_empty()));
...
static bool reduce_perf_run_axis_autograd_keepdims(reduce_perf_model *model, int32_t axis)
{
    gd_tensor out;
    gd_tensor dx;
    model->x.requires_grad = true;
                                     ...
                                     int iters)
{
    ...
    printf("[REDUCE_MEAN][%s] case=%s dtype=%s shape=", op, pcase->name, reduce_perf_dtype_name(pcase->dtype));
    reduce_perf_print_shape(pcase->rank, pcase->shape);
    printf(" axis=%d keepdims=%s axis_out=", pcase->axis, keepdims ? "true" : "false");
    if (keepdims) {
        reduce_perf_print_shape(model->axis_keep_rank, model->axis_keep_shape);
    } else {
...
static bool reduce_perf_run_case(const reduce_perf_case *pcase, int warmup, int iters)
{
    ...
    double axis_bwd_s;
    double axis_auto_s;
    double axis_keep_fwd_s;
    double axis_keep_bwd_s;
    double axis_keep_auto_s;
    double scalar_bytes;
    if (!reduce_perf_create_model(pcase, &model)) {
        return false;
    }
    scalar_bytes = (double)model.scalar_elem_size;
    printf("[REDUCE_MEAN] case=%s dtype=%s scalar_dtype=%s elems=%zu tensor=%.1fMiB axis_count=%zu\n",
           pcase->name,
           reduce_perf_dtype_name(pcase->dtype),
           reduce_perf_dtype_name(model.scalar_grad.dtype),
           model.x_count,
    ...
    axis_keep_fwd_s = reduce_perf_measure(&model, reduce_perf_run_axis_forward_keepdims, pcase->axis, warmup, iters);
    axis_keep_bwd_s = reduce_perf_measure(&model, reduce_perf_run_axis_backward_keepdims, pcase->axis, warmup, iters);
    axis_keep_auto_s = reduce_perf_measure(&model, reduce_perf_run_axis_autograd_keepdims, pcase->axis, warmup, iters);
    if (all_fwd_s > 0.0) {
        reduce_perf_print_result("all_fwd", pcase, &model, false, all_fwd_s, (double)model.x_bytes + scalar_bytes, warmup, iters);
    }
    if (all_bwd_s > 0.0) {
        reduce_perf_print_result("all_bwd_direct", pcase, &model, false, all_bwd_s, (double)model.x_bytes + scalar_bytes, warmup, iters);
    }
    if (all_auto_s > 0.0) {
    ...
    if (axis_fwd_s > 0.0) {
        reduce_perf_print_result("axis_fwd", pcase, &model, false, axis_fwd_s, (double)model.x_bytes + (double)model.axis_bytes, warmup, iters);
    }
    if (axis_bwd_s > 0.0) {
    ...
    if (axis_keep_fwd_s > 0.0) {
        reduce_perf_print_result("axis_fwd_keepdims",
                                 pcase,
                                 &model,
                                 true,
                                 axis_keep_fwd_s,
    ...
    if (axis_keep_bwd_s > 0.0) {
        reduce_perf_print_result("axis_bwd_direct_keepdims",
                                 pcase,
                                 &model,
                                 true,
                                 axis_keep_bwd_s,
    ...
    if (axis_keep_auto_s > 0.0) {
        reduce_perf_print_result("axis_fwd_bwd_autograd_keepdims",
                                 pcase,
                                 &model,
                                 true,
                                 axis_keep_auto_s,
    ...
    return all_fwd_s > 0.0 && all_bwd_s > 0.0 && all_auto_s > 0.0 && axis_fwd_s > 0.0 &&
           axis_bwd_s > 0.0 && axis_auto_s > 0.0 && axis_keep_fwd_s > 0.0 &&
           axis_keep_bwd_s > 0.0 && axis_keep_auto_s > 0.0;
}
...
int main(void)
{
    ...
    const char *profile = getenv("GD_REDUCE_MEAN_PERF_PROFILE");
    int warmup = reduce_perf_env_int("GD_REDUCE_MEAN_PERF_WARMUP", 10, 0, 10000);
    int iters = reduce_perf_env_int("GD_REDUCE_MEAN_PERF_ITERS", 100, 1, 1000000);
    bool ran = false;
    size_t i;
    printf("[REDUCE_MEAN] public API perf: warmup=%d iters=%d profile=%s\n",
           warmup,
           iters,
    ...
    if (!ran) {
        fprintf(stderr,
                "[REDUCE_MEAN][FAIL] no case matched GD_REDUCE_MEAN_PERF_PROFILE=%s\n",
