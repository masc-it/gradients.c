...
#define GD_CUDA_BACKEND_GENERATED_PATH "src/backends/cuda/backend_generated.c"
...
static gd_gen_op *gd_add_op(gd_gen_ops *ops, const char *name)
{
    ...
    if (name_len >= sizeof(op->name)) {
        return NULL;
    }
    op = &ops->items[ops->count];
    memset(op, 0, sizeof(*op));
    memcpy(op->name, name, name_len + 1U);
...
static bool gd_cuda_op_impl_exists(const gd_gen_op *op)
{
    char path[GD_GEN_PATH_MAX];
    if (op == NULL) {
        return false;
    }
    if (!gd_snprintf_ok(path, sizeof(path), "src/ops/%s/cuda_%s.cu", op->name, op->name)) {
        return false;
    }
    return gd_file_exists(path);
}

static int gd_generate_cuda_backend(const gd_gen_ops *ops)
{
    char *buf = NULL;
    size_t len = 0U;
    size_t cap = 0U;
    ...
    ok = ok && gd_append(&buf, &len, &cap,
                         "#include \"../../core/backend.h\"\n\n"
                         "/* @generated by tools/gen_ops.c; do not edit. */\n"
                         "/* CUDA unsupported stubs are omitted when src/ops/<op>/cuda_<op>.cu exists. */\n\n");
    for (i = 0U; ok && i < ops->count; ++i) {
        if (gd_cuda_op_impl_exists(&ops->items[i])) {
            continue;
        }
        if (ops->items[i].backend_unary) {
            char tmp[1600];
    ...
    if (!ok) {
        free(buf);
        fprintf(stderr, "gen_ops: failed to render CUDA backend stubs\n");
        return 1;
    }
    i = (uint32_t)gd_write_if_changed(GD_CUDA_BACKEND_GENERATED_PATH, buf, len);
    free(buf);
    return (int)i;
}
...
int main(int argc, char **argv)
{
    ...
    if (gd_generate_cuda_backend(&ops) != 0) {
        return 1;
    }
