...
#define CHECK(cond, msg)                                                     \
    do {                                                                    \
        if (!(cond)) {                                                      \
            fprintf(stderr, "test_tanh_ops failed: %s (%s:%d)\n", (msg),  \
                    __FILE__, __LINE__);                                    \
            exit(1);                                                        \
        }                                                                   \
    } while (0)
...
static void check_close(float have, float want, float tol, const char *msg)
{
    if (abs_f32(have - want) > tol) {
        fprintf(stderr,
                "test_tanh_ops failed: %s have=%.8f want=%.8f tol=%.8f\n",
                msg,
                (double)have,
                (double)want,
                (double)tol);
...
static gd_memory_config tanh_config(size_t tensor_bytes)
{
...
static void test_tanh_f32_forward_backward(void)
{
    enum { N = 13 };
    const int64_t shape[2] = {1, N};
    const float x_data[N] = {
        -20.0f, -12.0f, -8.0f, -4.0f, -1.0f, -0.125f, 0.0f,
        0.125f, 1.0f, 4.0f, 8.0f, 12.0f, 20.0f,
    };
    ...
    float got[N];
    gd_memory_config cfg = tanh_config(sizeof(x_data));
    gd_context *ctx = NULL;
    gd_tensor x;
    gd_tensor g;
    gd_tensor y;
    ...
    CHECK_OK(gd_tanh(ctx, &x, &y));
    CHECK_OK(gd_end_step(ctx));
    CHECK_OK(gd_synchronize(ctx));
    memset(got, 0, sizeof(got));
    CHECK_OK(gd_tensor_read(ctx, &y, got, sizeof(got)));
    for (i = 0U; i < N; ++i) {
        check_close(got[i], tanhf(x_data[i]), 8.0e-6f, "f32 tanh forward");
    }

    CHECK_OK(gd_begin_step(ctx, GD_SCOPE_TRAIN, gd_batch_empty()));
    CHECK_OK(gd_tanh_backward(ctx, &x, &g, &dx));
    CHECK_OK(gd_end_step(ctx));
    CHECK_OK(gd_synchronize(ctx));
    memset(got, 0, sizeof(got));
    CHECK_OK(gd_tensor_read(ctx, &dx, got, sizeof(got)));
    for (i = 0U; i < N; ++i) {
        float t = tanhf(x_data[i]);
        check_close(got[i], g_data[i] * (1.0f - t * t), 1.0e-5f, "f32 tanh direct backward");
    }

    x.requires_grad = true;
    CHECK_OK(gd_begin_step(ctx, GD_SCOPE_TRAIN, gd_batch_empty()));
    CHECK_OK(gd_tanh(ctx, &x, &y_auto));
    ...
    for (i = 0U; i < N; ++i) {
        float t = tanhf(x_data[i]);
        check_close(got[i], g_data[i] * (1.0f - t * t), 1.0e-5f, "f32 tanh autograd backward");
    }

    gd_context_destroy(ctx);
}

static void test_tanh_f16_forward_backward(void)
{
    enum { N = 17 };
    const int64_t shape[2] = {1, N};
    ...
    CHECK_OK(gd_tensor_read(ctx, &y, got, sizeof(got)));
    for (i = 0U; i < N; ++i) {
        float want = f16_bits_to_f32(f32_to_f16_bits(tanhf(f16_bits_to_f32(x_data[i]))));
        check_close(f16_bits_to_f32(got[i]), want, 1.2e-3f, "f16 tanh forward");
    }
    ...
    for (i = 0U; i < N; ++i) {
        ...
        float t = tanhf(x_f);
        float want = f16_bits_to_f32(f32_to_f16_bits(g_f * (1.0f - t * t)));
        check_close(f16_bits_to_f32(got[i]), want, 1.5e-3f, "f16 tanh direct backward");
    }
    ...
    CHECK_OK(gd_tensor_read(ctx, &dx_auto, got, sizeof(got)));
    for (i = 0U; i < N; ++i) {
        float saved = f16_bits_to_f32(f32_to_f16_bits(tanhf(f16_bits_to_f32(x_data[i]))));
        float g_f = f16_bits_to_f32(g_data[i]);
        float want = f16_bits_to_f32(f32_to_f16_bits(g_f * (1.0f - saved * saved)));
        check_close(f16_bits_to_f32(got[i]), want, 1.5e-3f, "f16 tanh autograd backward");
    }

    gd_context_destroy(ctx);
}

static void test_tanh_rejects_i32(void)
{
    const int64_t shape[1] = {1};
    gd_memory_config cfg = tanh_config(4U);
    gd_context *ctx = NULL;
    gd_tensor x;
    gd_tensor y;
    CHECK_OK(gd_context_create(&cfg, &ctx));
    ...
    CHECK_OK(gd_context_seal_params(ctx));
    CHECK_STATUS(gd_tanh(ctx, &x, &y), GD_ERR_UNSUPPORTED);
    gd_context_destroy(ctx);
}

int main(void)
{
    test_tanh_f32_forward_backward();
    test_tanh_f16_forward_backward();
    test_tanh_rejects_i32();
    printf("test_tanh_ops: ok\n");
    return 0;
}
