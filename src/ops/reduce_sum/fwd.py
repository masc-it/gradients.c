...
"""Live gradients.c reduce_sum forward correctness check against PyTorch."""
...
int main(int argc, char **argv)
{
    ...
    size_t x_count, out_count, elem_size, x_bytes;
    void *x_data = NULL;
    void *out_data = NULL;
    gd_context *ctx = NULL;
    ...
    CHECK(ctx, gd_tensor_read(ctx, &out, out_data, out_count * elem_size));
    if (write_file(argv[2], out_data, out_count * elem_size) != 0) { goto fail; }
    rc = 0;
    goto done;
fail:
    rc = 1;
...
def make_cases() -> list[Case]:
    ...
    profile = os.environ.get("GD_REDUCE_SUM_FWD_PROFILE", "smoke")
    if profile == "smoke":
        return smoke
    if profile == "classic":
        return classic
    ...
    if not selected:
        raise SystemExit(f"unknown GD_REDUCE_SUM_FWD_PROFILE={profile!r}")
    return selected
...
def run_case(runner: Path, root: Path, tmp: Path, case: Case) -> bool:
    ...
    if proc.returncode == 77:
        print(f"reduce_sum fwd skipped case={case.name}: {proc.stdout.strip()}")
        return True
    if proc.returncode != 0:
        print(proc.stdout, end=""); print(proc.stderr, end="")
        raise RuntimeError(f"runner failed case={case.name} rc={proc.returncode}")
    actual = np.fromfile(out_path, dtype=np_dtype).reshape(output_shape(case))
    want = ref.detach().cpu().contiguous().numpy().astype(np_dtype, copy=False).reshape(output_shape(case))
    ...
    tol = 2.0e-2 if case.dtype == "f16" else 2.0e-4
    ok = max_abs <= tol
    print(f"reduce_sum fwd actual {'ok' if ok else 'FAIL'} case={case.name} dtype={case.dtype} "
