...
#include "powlu_split_linear_impl.h"
...
typedef struct gd_powlu_split_linear_shape_info {
    uint32_t rank;
    ...
    int64_t hidden;
    int64_t out_cols;
} gd_powlu_split_linear_shape_info;

static gd_status gd_powlu_split_linear_validate_m(gd_context *ctx, float m)
{
    if (!isfinite(m) || m <= 0.0f || m >= 10.0f) {
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "powlu_split_linear m must satisfy 0 < m < 10");
    }
    return GD_OK;
}

static gd_status gd_powlu_split_linear_build_shape(gd_context *ctx,
                                                   const gd_tensor *x12,
                                                   const gd_tensor *w,
                                                   const gd_tensor *bias,
                                                   gd_powlu_split_linear_shape_info *info)
{
    uint32_t axis;
    int64_t rows = 1;
    ...
    if (x12->dtype != GD_DTYPE_F16 || w->dtype != GD_DTYPE_F16 ||
        ...
        return gd_context_set_error(ctx,
                                    GD_ERR_UNSUPPORTED,
                                    "powlu_split_linear currently supports f16 tensors only");
    }
    if (x12->rank < 1U || x12->rank > GD_MAX_DIMS || w->rank != 2U ||
        (bias != NULL && bias->rank != 1U)) {
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "powlu_split_linear expects x12 [..., 2H], w [H, N], bias [N]");
    }
    if (x12->shape[x12->rank - 1U] <= 0 ||
        ...
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "powlu_split_linear shape mismatch");
    }
    info->rank = x12->rank;
...
static gd_status gd_powlu_split_linear_validate_common(gd_context *ctx,
                                                       const gd_tensor *x12,
                                                       const gd_tensor *w,
                                                       const gd_tensor *bias,
                                                       float m,
                                                       gd_powlu_split_linear_shape_info *info)
{
    ...
    st = gd_powlu_split_linear_validate_m(ctx, m);
    if (st != GD_OK) {
        return st;
    }
    st = gd_powlu_split_linear_build_shape(ctx, x12, w, bias, info);
    if (st != GD_OK) {
        return st;
    }
    if (!gd_tensor_is_contiguous(x12) || w->strides[1] != 1 || w->strides[0] <= 0 ||
        ...
        return gd_context_set_error(ctx,
                                    GD_ERR_UNSUPPORTED,
                                    "powlu_split_linear requires contiguous x12 and row-strided weight/bias");
    }
    return GD_OK;
}

static gd_status gd_powlu_split_linear_validate_activation(gd_context *ctx,
                                                           ...
                                                           const gd_powlu_split_linear_shape_info *info)
{
    ...
    if (activation->dtype != GD_DTYPE_F16 || activation->rank != info->rank ||
        ...
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "powlu_split_linear activation shape/layout mismatch");
    }
...
static gd_status gd_powlu_split_linear_validate_grad_out(gd_context *ctx,
                                                         ...
                                                         const gd_powlu_split_linear_shape_info *info)
{
    ...
    if (grad_out->dtype != GD_DTYPE_F16 || grad_out->rank != info->rank ||
        ...
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "powlu_split_linear backward grad_out shape/layout mismatch");
    }
    for (axis = 0U; axis < info->rank; ++axis) {
    ...
    if (grad_out->rank == 2U) {
        if (grad_out->strides[0] <= 0) {
            return gd_context_set_error(ctx,
                                        GD_ERR_UNSUPPORTED,
                                        "powlu_split_linear backward requires row-strided grad_out");
        }
    } else if (!gd_tensor_is_contiguous(grad_out)) {
        return gd_context_set_error(ctx,
                                    GD_ERR_UNSUPPORTED,
                                    "powlu_split_linear backward rank-N grad_out must be contiguous");
    }
    return GD_OK;
}
...
{
    return gd_powlu_split(ctx, x12, m, activation);
}

static bool gd_powlu_split_linear_recompute_enabled(void)
{
    const char *env = getenv("GD_POWLU_SPLIT_LINEAR_RECOMPUTE");
    return env != NULL && env[0] != '\0' && env[0] != '0';
}

static gd_status gd_powlu_split_linear_ensure_activation(gd_context *ctx,
                                                         const gd_tensor *x12,
                                                         ...
                                                         const gd_tensor **out_activation)
{
    ...
    st = gd_powlu_split_linear_recompute_activation(ctx, x12, m, computed);
    if (st != GD_OK) {
        return st;
    }
                                ...
                                gd_tensor *out)
{
    ...
    st = gd_powlu_split_linear_validate_common(ctx, x12, w, bias, m, &info);
    if (st != GD_OK) {
        return st;
    }

    was_recording = gd_is_grad_enabled(ctx);
    save_activation = was_recording && !gd_powlu_split_linear_recompute_enabled();
    if (was_recording) {
    ...
    st = gd_powlu_split(ctx, x12, m, &activation);
    if (st == GD_OK) {
                                                         ...
                                                         gd_tensor *grad_bias)
{
    ...
    if (need_bias && bias == NULL) {
        return gd_context_set_error(ctx,
                                    GD_ERR_INVALID_ARGUMENT,
                                    "powlu_split_linear grad_bias requested without bias");
    }
    st = gd_powlu_split_linear_validate_common(ctx, x12, w, bias, m, &info);
    if (st != GD_OK) {
        return st;
...
gd_status gd_powlu_split_linear_backward(gd_context *ctx,
                                         ...
                                         gd_tensor *grad_bias)
{
    return gd_powlu_split_linear_backward_with_activation(ctx,
                                                          x12,
                                                          NULL,
                                                          w,
                                                          bias,
