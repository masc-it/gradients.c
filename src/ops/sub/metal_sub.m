...
static id<MTLComputePipelineState> gd_sub_row_bcast_pso(gd_backend *backend)
{
    return (__bridge id<MTLComputePipelineState>)backend->binary_row_bcast_pso[GD_OP_SUB];
}
...
{
    return gd_metal_binary_dispatch(backend,
                                    x,
                                    y,
                                    out,
                                    gd_sub_pso(backend),
                                    gd_sub_bcast_pso(backend),
                                    gd_sub_row_bcast_pso(backend));
}
